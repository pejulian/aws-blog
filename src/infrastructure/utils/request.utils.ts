...
import { MalformedCallbackInputError } from "../errors/MalformedCallbackInputError.js";

export type QueryStringParameters =
  APIGatewayProxyEvent["queryStringParameters"];

export type Cookies = Readonly<{
  [key: string]: string;
}>;
...
export class RequestUtils {
  ...
  getRequestedURL(event: CloudFrontRequestEvent): string {
    let { uri: path, querystring } = event.Records[0].cf.request;
  ...
  parseRequestedURLFromState(event: CloudFrontRequestEvent): string {
    ...
    if (!queryStringParams?.state) {
      throw new MalformedCallbackInputError(
        "state is a required query string parameter."
      );
    }
    ...
    if (!state.requestedURL) {
      throw new MalformedCallbackInputError(
        "querystring.state.requestedURL is a required field."
      );
    }
